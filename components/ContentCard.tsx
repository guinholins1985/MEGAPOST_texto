...
export const ContentCard: React.FC<ContentCardProps> = ({ title, data }) => {
        ...
        if (Array.isArray(data)) {
            return (
                <ul className="space-y-4">
                    {data.map((item, index) => (
                        <li key={index} className="text-text-primary bg-base-100 p-3 rounded-md border border-base-300">
                           {typeof item === 'string' && <p>{item}</p>}
                           {typeof item === 'object' && item !== null && (
                               <div className="space-y-1">
                                   {Object.entries(item).map(([key, value]) => (
                                       <div key={key}>
                                           <strong className="font-semibold text-brand-light capitalize">{key}: </strong>
    ...
    return (
        <div className="bg-base-300/50 rounded-lg p-4 flex flex-col h-full">
            <div className="flex justify-between items-center mb-3">
                <h4 className="font-bold text-lg text-brand-secondary">{title}</h4>
                <button
                    onClick={handleCopy}
                    className={`px-3 py-1 text-xs font-medium rounded-md flex items-center gap-1.5 transition-colors ${copied ? 'bg-green-600 text-white' : 'bg-base-300 hover:bg-brand-dark text-text-primary'}`}
                >
